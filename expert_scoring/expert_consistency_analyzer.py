...
def unify_schema(df_e1: pd.DataFrame, df_e2: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
    ...
    - Preserve all scoring columns (C1..C6, O7, O8, Process/Outcome/Total)
    ...
    # Identify scoring columns present in both
    possible_score_cols = [
        'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'O7', 'O8',
        'Process Score', 'Outcome Score', 'Total Score'
    ]
...
def plot_icc_by_condition(icc_df: pd.DataFrame, out_dir: str) -> str:
    ...
    plt.title('ICC(2,k) by Condition')
    plt.ylabel('ICC(2,k)')
    plt.legend()
    out_path = os.path.join(out_dir, 'icc_by_condition_summary_scores.png')
    plt.tight_layout()
...
def main_step1() -> None:
    ...
    # For step 1, we focus on the three summary scores
    summary_cols = [col for col in ['Process Score', 'Outcome Score', 'Total Score'] if col in score_cols]
    if len(summary_cols) == 0:
        raise ValueError("No summary score columns found. Expected at least one of: 'Process Score', 'Outcome Score', 'Total Score'.")
    ...
    print("=== Basic descriptive statistics (Summary Scores) ===")
    ...
    os.makedirs(out_dir, exist_ok=True)
    df_e1_clean.to_csv(os.path.join(out_dir, 'E1_clean.csv'), index=False)
    df_e2_clean.to_csv(os.path.join(out_dir, 'E2_clean.csv'), index=False)
    desc.to_csv(os.path.join(out_dir, 'summary_descriptives.csv'), index=False)
...
def main_step2() -> None:
    ...
    out_dir = os.path.join(get_project_dir(), 'intermediate')
    os.makedirs(out_dir, exist_ok=True)
    # Re-order rows so that Overall -> AI -> NoAI, and within each, Process -> Outcome -> Total
    scope_order_map = {
        'Overall': 0,
        'Condition=AI': 1,
        'Condition=NoAI': 2,
    ...
    icc_out_csv = os.path.join(out_dir, 'icc_results.csv')
