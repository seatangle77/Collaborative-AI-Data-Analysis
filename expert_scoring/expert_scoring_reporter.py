...
def build_html() -> str:
    ...
    # Load tables (reuse if already generated by previous scripts)
    desc_csv = os.path.join(inter_dir, 'summary_descriptives.csv')
    icc_csv = os.path.join(inter_dir, 'icc_results.csv')
    within_csv = os.path.join(inter_dir, 'within_group_tests.csv')
    ...
    # Images
    icc_summary_img = os.path.join(plots_dir, 'icc_by_condition_summary_scores.png')
    paired_imgs = list_existing_images(plots_dir, patterns=['paired_lines_'])
    diff_bar_imgs = list_existing_images(plots_dir, patterns=['diff_bar_'])
    diff_box_imgs = list_existing_images(plots_dir, patterns=['diff_box_'])
    ...
    html = [
        ...
        '<div class="section">',
        '<h2>1. 描述性统计</h2>',
        '<div class="hint">说明：本表给出两位专家（E1/E2）在三类分数（Process/Outcome/Total）上的分布特征，包括均值、标准差及分位数；同时提供按条件（AI/NoAI）分组的统计。用于观察分布是否存在天花板/地板效应与条件差异。若表格为空，请先运行前置脚本生成中间结果：expert_consistency_analyzer.py、within_group_analysis.py。</div>',
        safe_to_html(df_desc),
        '</div>',

        '<div class="section">',
        '<h2>2. 专家间一致性（ICC）</h2>',
        '<div class="hint">说明：ICC(2,k) 为双向随机效应、绝对一致、平均评分。一致性阈值常用：≥0.70 可合并；0.50–0.70 中等一致性（谨慎解读）；<0.50 不建议合并。表中CI为95%置信区间，若跨0则一致性证据不足。Overall 使用全部样本；Condition=AI/NoAI 分别仅使用对应条件的样本。</div>',
        safe_to_html(df_icc),
    ]

    if os.path.exists(icc_summary_img):
        rel = os.path.relpath(icc_summary_img, base_dir)
        html += [f'<img class="figure" src="{rel}" alt="ICC by Condition (Summary Scores)"/>']
    ...
    if paired_imgs:
        html += [
            ...
            '<div class="hint">说明：每条线代表一个组，从 NoAI 指向 AI。可直观看到各组方向与幅度；若线条方向杂乱，说明不同组变化趋势不一致。</div>',
            '<div class="grid">'
        ]
    ...
    if diff_bar_imgs:
        html += [
            ...
            '<div class="hint">说明：每根柱表示一个组的差值（AI−NoAI），上方红线为整体均值，虚线为95%CI。整体均值与零线的关系体现总体差异方向与大小。</div>',
            '<div class="grid">'
        ]
    ...
    if diff_box_imgs:
        html += [
            ...
            '<div class="hint">说明：展示所有组的差值分布与离群点，红线为均值。与柱状图互为补充，有助于识别分布偏态与极端值。</div>',
            '<div class="grid">'
        ]
    ...
    html += [
        ...
        '<li>本数据：Process、Total 整体一致性不足；Outcome 仅在 NoAI 条件下达到可合并水平。</li>',
...
def main() -> None:
    base_dir = get_project_dir()
    out_path = os.path.join(base_dir, 'expert_scoring_report.html')
