...
SummaryScores = ['Process Score', 'Outcome Score', 'Total Score']
...
def plot_paired_lines(df_exp: pd.DataFrame, expert_name: str, out_dir: str) -> List[str]:
    ...
    for score in SummaryScores:
        ...
        plt.title(f"{expert_name} - {score} (Paired AI vs NoAI)")
        plt.ylabel(score)
        plt.legend()
        plt.tight_layout()
        out_path = os.path.join(out_dir, f"paired_lines_{expert_name.replace(' ', '_')}_{score.replace(' ', '_')}.png")
...
def plot_diff_bar(df_exp: pd.DataFrame, expert_name: str, out_dir: str) -> List[str]:
    ...
    for score in SummaryScores:
        ...
        plt.title(f"{expert_name} - {score} (Difference per Group)")
        plt.tight_layout()
        out_path = os.path.join(out_dir, f"diff_bar_{expert_name.replace(' ', '_')}_{score.replace(' ', '_')}.png")
        plt.savefig(out_path, dpi=150)
        plt.close()
...
def plot_diff_box(df_exp: pd.DataFrame, expert_name: str, out_dir: str) -> List[str]:
    ...
    for score in SummaryScores:
        ...
        plt.axhline(mean_diff, color='#e41a1c', linestyle='-', linewidth=1.5, label='Mean diff')
        plt.ylabel(f"AI - NoAI ({score})")
        plt.title(f"{expert_name} - {score} (Difference Boxplot)")
        plt.tight_layout()
        out_path = os.path.join(out_dir, f"diff_box_{expert_name.replace(' ', '_')}_{score.replace(' ', '_')}.png")
        plt.savefig(out_path, dpi=150)
        plt.close()
        out_paths.append(out_path)
    return out_paths
...
def main() -> None:
    ...
    os.makedirs(out_dir, exist_ok=True)
    out_csv = os.path.join(out_dir, 'within_group_tests.csv')
    ...
    print("=== Within-group paired tests (AI vs NoAI) ===")
